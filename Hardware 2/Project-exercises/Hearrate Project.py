...
def detect_peaks(data, sample_rate=250):
    ...
    for i in range(1, len(data)):  # Assuming we've already processed one data point
        ...
        threshold_off = minimum + (maximum - minimum) * 0.70
        if not counting and prev_slope > 0 and slope <= 0 and curr > threshold_on:
            counting = True
            peaks.append(sample)
        
        if counting and curr < threshold_on:
            counting = False
...
def lpf2(data, filtering_amount=63):
    lastvals = []
    filtered_range = []
    for value in data:
